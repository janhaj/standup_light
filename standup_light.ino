...
RgbColor redHalf(colorSaturation / 10, 0, 0);
RgbColor green(0, colorSaturation, 0);
RgbColor greenHalf(0, colorSaturation / 10, 0);
...
void loop()
{
    for(int i = 0; i < 36; i++)
    {
      ...
      {
        if (led == i % PixelCount)
          strip.SetPixelColor(led, available ? green : red);
        else if(led + 1 == i % PixelCount)
          strip.SetPixelColor(led, available ? greenHalf : redHalf);
        else 
    ...
    setAll(available ? green : red);
    delay(200);
    setAll(available ? greenHalf : redHalf);
    delay(1000); 
    available = !available;
}
